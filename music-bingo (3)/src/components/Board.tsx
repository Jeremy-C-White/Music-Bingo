...
export default function Board() {
  ...
  const findNowPlayingOnBoard = () => {
    if (!gameState?.nowPlaying) {
      showToast("⏳ Host hasn't called a track yet");
      return;
    }
    const currentSong = gameState.nowPlaying;
    const idx = boardSongs.findIndex(s => s === currentSong);
    if (idx !== -1) {
      setHighlightIdx(idx);
      const { title } = splitSong(currentSong);
      showToast(<span>🎯 Found on your board: <b>{title}</b>!</span>);
      setTimeout(() => setHighlightIdx(null), 3000);
    } else {
      showToast("❌ Not on your board - keep listening!");
    }
  };
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-xs font-black text-white truncate">{currentTitle} {currentArtist && <span className="text-white/60 font-medium">— {currentArtist}</span>}</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button 
                  onClick={findNowPlayingOnBoard}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-xs font-bold text-white transition-all cursor-pointer"
                >
                  <Search className="w-3.5 h-3.5 text-[#33d8ff]" /> Check My Board
                </button>
              </div>
            </div>
